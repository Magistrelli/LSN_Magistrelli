...
class Chromo: public Path {
    ...
    //constructors
    Chromo();
    Chromo(DataVett* pos, Random* rnd, int ichr, int rank);//rank for parallel computing
    //destructor
...
class Population {
    ...
    double MinLoss;	//Loss of the shortest path ever visited
    int ParSize,ParRank;//size and rank for parallel code

    //internal functions
    void Crossover(int,int);	//crossover operator
    int Selection() const;	//sort one parent (low cost frequently)
    ...
    void WriteEnd();		//write final configuration (all chromosomes)
    
    public:
    //constructors
    Population();
    Population(Random* rnd);	//for single core operations
    Population(Random* rnd, int size, int rank);//parellel computing
