...
void Population::Evolution() {
    int sel[2],igen=1,nnmig=0;	//parents chromosomes number
    int count=0,incount=0;	//#gen of non improving MinLoss
    Chromo newChr[Nchr];	//brand new generation
    double *mins,writtenMin=1e3;//min recv, min in file best
    ...
    while(count<GenMax){
      ...
      if((ParSize>1)&&((igen+1)%Nmigr==0)){
        //see if someone else has a better MinLoss
	...
	*Chr[i]=newChr[i];	//replace old generation
	Chr[i]->LossCost();	//and evaluate new loss function's values
      ...
      if(Chr[0]->GetLoss()<MinLoss){
	Chr[0]->WriteBest(*Loc);
	MinLoss=Chr[0]->GetLoss();
	if(ParSize>1) writtenMin=MinLoss;
	count=0,incount=0;
      } else {incount++;}
      if((ParSize>1)&&(Chr[0]->GetLoss()<writtenMin)){
	Chr[0]->WriteBest(*Loc);
	writtenMin=Chr[0]->GetLoss();
      }
      if((igen+2)%Nmigr==0){	//two gen before Migration
...
void Population::Crossover(int sel1, int sel2) {
    bool equal=1;
    int pos=int(Rnd->Rannyu(2.,Ncit-1.));
    int m=Ncit-pos,search,find;
    int appo[2*m];
	...
	for(int i=0; i<2; ++i){
	  ...
	  while(find<m){
	    ...
	    if(search>=Ncit){
		cerr<<endl<<"Error in crossover! removed elements from "<<i+1<<" chromo didn't find in the other one!!"<<endl;
		exit(-3);
	    }
	    
	    for(int j=0; j<m; ++j){
		if(p[(i+1)%2]->GetGene(search)==p[i]->GetGene(pos+j)){
		    appo[i*m+find]=p[i]->GetGene(pos+j);
		    find++;
		    j=m;
		}
	    }
...
//Select individual with weight 1/loss (ordering not needed)
//with this implementation the more better is a path respect to others the more it will reproduce itself
...
void Population::WriteLength(int igen) const {
    ...
    if(igen==1){OutLoss.open("results/path"+*Loc+"lenght");
		OutLoss.close();}
...
//write final chromosomes and final seed in default file
